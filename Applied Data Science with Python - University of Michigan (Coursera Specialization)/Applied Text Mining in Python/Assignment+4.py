...
def similarity_score(s1, s2):
    ...
    s = []
    for synset in s1:
        sim = [synset.path_similarity(x) for x in s2 if synset.path_similarity(x) is not None]
        if sim:
            s.append(max(sim))
    return np.mean(s)
...
def most_similar_docs():
    ...
    D1 = paraphrases.loc[np.argmax(sim_scores),'D1']
    D2 = paraphrases.loc[np.argmax(sim_scores),'D2']
    similarity_score = max(sim_scores)
...
def label_accuracy():
    ...
    sim_scores = pd.Series([document_path_similarity(x,y) for x,y in zip(paraphrases['D1'], paraphrases['D2'])])
    return accuracy_score(paraphrases['Quality'], np.where(sim_scores.values>0.75, 1, 0))
...
def topic_distribution():
    ...
    return list(ldamodel[gensim.matutils.Sparse2Corpus(vect.transform(new_doc), documents_columns=False)])[0]
