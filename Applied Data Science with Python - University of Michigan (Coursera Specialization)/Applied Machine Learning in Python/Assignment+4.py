...
def blight_model():
    ...
    label_encoder = LabelEncoder()
    label_encoder.fit(train['violation_description'].append(test['violation_description'], ignore_index=True))
    train['violation_description'] = label_encoder.transform(train['violation_description'])
    test['violation_description'] = label_encoder.transform(test['violation_description'])
    
    features = ['disposition', 'violation_code', 'violation_description']
    X = train[features]
    y = train['compliance']
    test = test[features]
    ...
    y_decision_fn_scores_auc = grid_clf_auc.predict(test) 
    result = pd.DataFrame(y_decision_fn_scores_auc,index=test.index,columns=['compliance'])
